...
public class Main implements IClientMessageHandler {
    ...
    public static void main(String[] args) {
        ...
        if (args.length > 0) {
            if (args[0].equals("-serverOnly")) {
                c.logType = 1;
                new BaseServer(c, new ComServer(c));
            }
        } else {
            new Main(c);
        }
    ...
    public void createOrJoin() {
        ...
        panel.add(leftPanel);
        inputs = new JPanel(new GridLayout(5, 2));
                    ...
                    switch (e.getKeyCode()) {
                        ...
                        case 38://arrow up
                            if (playerList.getSelectedIndex() > 0) {
                                ...
                                comClient.queueOut(MessagePlayerList.playerOrderChanged(players));
                            }
                            break;
                        case 34://page down
                        case 40://arrow down
        ...
        create.addActionListener(e -> {
            ...
            if (!playername.getText().trim().equals("") && !port.getText().trim().equals("")) {
                server = new BaseServer(c, new ComServer(c));
                int nmbRetries = 15;
                for (int i = 0; !server.comServer.listening && i < nmbRetries; i++) {
                ...
                if (server.comServer.listening) {
                    ...
                    comClient = new ComClient(c, this);
                }
            }
        });
    ...
    private void createAdminLobbyMenu(JPanel inputs, JPanel rightPanel) {
        isAdmin = true;

        JPanel npcPanel = new JPanel(new GridLayout(5, 2));
        JPanel npcPanel2 = new JPanel(new GridLayout(1, 2));
        npcPanel2.add(new JLabel("NPC Name:"));
        JTextField npcName = new JTextField();
        npcPanel.add(new JLabel());
        npcPanel2.add(npcName);
        npcPanel.add(npcPanel2);
        JButton addNPC = new JButton("neuer Spieler");
        npcPanel.add(addNPC);
        addNPC.addActionListener(e -> addNPC(npcName.getText()));
        leftPanel.add(npcPanel);
        ...
        playerList.addListSelectionListener(p -> comClient.queueOut(new MessageSetStartPlayer(playerList.getSelectedIndex())));
        setConfigMenu();
    }
    ...
    private void addNPC(String npcName) {
        if(npcName!=null && !npcName.equals("")) {
            new NPCClient(c, npcName);
        }
    }

    private void startGameServer(Statics.game game) {
        comClient.queueOut(new MessageStartGame(game.name()));
    }
    ...
    public void handleInput(Message message) {
        ...
        log.info("handled message: " + message.getCommand());
    }
    ...
    private void handleConfiguration(Message message) {
        ...
        configPanelPassive.add(new JTextArea(Helper.removeJsonChars(c.other.toPrettyJson())));
        if (c.other.lastGame.equals(Statics.game.SKAT.name())) {
            configPanelPassive.add(new JTextArea(Helper.removeJsonChars(c.skat.toPrettyJson())));
        } else {
            configPanelPassive.add(new JTextArea(Helper.removeJsonChars(c.doko.toPrettyJson())));
        }
    ...
    private void handlePlayersInLobby(Message message) {
        try {
            ...
            if (players.size() == 1) {
                createAdminLobbyMenu(inputs, rightPanel);
            }
    ...
    final HashMap<String, BufferedImage> rawImages = new HashMap<>();
    final HashMap<String, ImageIcon> rawIcons = new HashMap<>();
    boolean ready;
    ...
    private void setConfigMenu() {
        ...
        comClient.queueOut(new MessageConfiguration(c));
