...
import base.game.skat.SkatEndDialog;
import base.game.skat.SortHand;
...
public class SkatClient extends BaseClient implements IClientMessageHandler, IDialogInterface {
    ...
    protected void createGameSpecificButtons() {
        ...
        pass.addActionListener(e -> {
            handler.queueOut(new MessagePassen(c.connection.name));
    ...
    private void handleGrandHand(Message message) {
        ...
        gh.addActionListener(e -> {
            handler.queueOut(new MessageGrandHand(c.connection.name, true));
            middlePanel.removeAll();
        ...
        ramsch.addActionListener(e -> {
            handler.queueOut(new MessageGrandHand(c.connection.name, false));
            middlePanel.removeAll();
            middlePanel.revalidate();
            middlePanel.repaint();
            gameMessageLabel.setText("");
    ...
    private void handleRamschSkat() {
        ...
        button_ok.addActionListener(e -> {
            handler.queueOut(MessageSkatCards.SkatCards(c.connection.name, Arrays.asList(exchangeCards)));
            middlePanel.removeAll();
            middlePanel.repaint();
            selectCards = false;
        });
        ...
        button_skat.addActionListener(e -> {
            ...
            selectCards = true;
            button_ok.setVisible(true);
            handler.queueOut(new MessageGetSkat(c.connection.name));
        });

        button_schieben.addActionListener(e -> {
            handler.queueOut(new MessageSchieben(c.connection.name));
            middlePanel.removeAll();
            middlePanel.revalidate();
            middlePanel.repaint();
        });
    ...
    private void handleSelectGame() {
        ...
        button_ok.addActionListener(e -> {
            handler.queueOut(new MessageGameSelected(c.connection.name, selectedGame, handSpiel, ouvert));
            if (!handSpiel) {
                handler.queueOut(MessageSkatCards.SkatCards(c.connection.name, Arrays.asList(exchangeCards)));
            }
            middlePanel.removeAll();
