...
public class DokoClient extends BaseClient implements IClientMessageHandler, IDialogInterface {
    ...
    @Override
    protected void setGameSpecificButtons() {

        controlPanel.removeAll();
        JButton vorbehalt = new JButton("OK");
        vorbehalt.addActionListener(e -> {
            handler.queueOut(new MessageGameSelected(players.indexOf(c.connection.name), selectedGame));
        ...
        for(int i = 0 ;i<100;i++){
            System.out.println(hand.stream().filter(p -> p.trump).count());
            try {
                Thread.sleep(1);
        ...
        if (hand.stream().filter(p -> p.trump).count() < 4) {
            controlPanel.add(sortArmut);
        }
        if (hand.stream().filter(p -> p.kind.equals(Statics.KOENIG)).count() > 4) {
            controlPanel.add(sortKoenige);
        }
        if (hand.stream().filter(p -> p.kind.equals(Statics.DAME) && p.suit.equals(Statics.KREUZ)).count() > 1) {
                    ...
                    if (wait4Player) {
                        if (c.other.bedienen) {
                            ...
                            if (allowed) {
                                ...
                                handler.queueOut(new MessagePutCard(players.indexOf(c.connection.name), card));
                                if (c.ui.redrawCards) {
                                    createCardButtons(hand);
                                } else {
    ...
    private void handleAnnounceSpectator(Message message) {
        ...
        if (players.size() > 4 && players.get(spectator).equals(c.connection.name)) {
            handler.queueOut(new MessageDisplayMessage("Du bist jetzt Zuschauer"));
            hand.clear();
            clearPlayArea();
            createCardButtons(hand);
        }
    ...
    private void handleGetArmut() {
        if (JOptionPane.showConfirmDialog(mainFrame, "Armut mitnehmen?",
            ...
            handler.queueOut(new MessageGetArmut(players.indexOf(c.connection.name), true));
        } else {
            handler.queueOut(new MessageGetArmut(players.indexOf(c.connection.name), false));
        }
    }
    ...
    private void setAcceptArmutReturn() {
        ...
        button.addActionListener(e -> {
            ...
            handler.queueOut(new MessageCardsReturned());
            middlePanel.removeAll();
        ...
        if (hand != null && hand.size() > 0) {
            ...
            if (selectedGame == NORMAL
                ...
                if (hand.stream().filter(p -> p.suit.equals(Statics.KARO)
                    ...
                    handler.queueOut(new MessageSchweinExists());
                    schweinExists = true;
                } else {
                    schweinExists = false;
                }
    ...
    public void quitEnd() {
        ...
        handler.queueOut(new MessageReadyForNextRound(players.indexOf(c.connection.name)));
    }
    ...
    private void setSendCardButton(String receiver, String buttonText) {
        ...
        sendCardsButton.addActionListener(e -> {
            handler.queueOut(new MessageSendCards(Arrays.asList(exchangeCards), receiver));
            selectCards = false;
            controlPanel.setVisible(false);
            middlePanel.removeAll();
            middlePanel.setVisible(false);
